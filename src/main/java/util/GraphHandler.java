...
//            }
    ...
    public static Hashtable<Integer, LinkedList<Integer>> removeNodeFromAdjMap(Hashtable<Integer, LinkedList<Integer>> adjMap, Integer changeNode) {
        LinkedList<Integer> delQue=adjMap.get(changeNode);
        adjMap.remove(changeNode);
        for (Integer adNode : delQue) {
            adjMap.get(adNode).remove(changeNode);
        }
        return adjMap;
    }
